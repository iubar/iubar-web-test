...
abstract class Web_TestCase extends Root_TestCase {
	...
	protected static $screenshots = [];

	protected static $dump_files = [];
	...
	public static function setUpBeforeClass(): void {
		...
		switch (self::$browser) {
			case self::PHANTOMJS:
				...
				$cli_args = [
					// TODO: da verificare se svolgono il loro compito
					'--debug' => 'true',
					'--webdrive' => '',
					'--loglevel' => 'DEBUG',
					'--webdriver-logfile' => self::$logs_path . '/phantomjsdriver.log'
				];
				$ghostdriver_cli_args = [
					'--loglevel' => 'DEBUG',
			...
			default:
				self::$climate->error(
					"Browser '" . self::$browser . "' not supported. (you should set the BROWSER global var with a supported browser name)"
				);
				exit(1);
		}
		...
		try {
			...
			if (self::$browser != self::SAFARI) {
				// set some timeouts
				self::$webDriver->manage()->timeouts()->pageLoadTimeout(60); // Set the amount of time (in seconds) to wait for a page load to complete before throwing an error

				self::$climate->info('setScriptTimeout()...');
				self::$webDriver->manage()->timeouts()->setScriptTimeout(240); // Set the amount of time (in seconds) to wait for an asynchronous script to finish execution before throwing an error.
			}
	 ...
	 */
	protected function click(string $xpath, float $wait = 0.5) {
		$this->getWd()->findElement(WebDriverBy::xpath($xpath))->click();

		$this->getWd()->manage()->timeouts()->implicitlyWait($wait); // FIXME: non compatibile con Safari (soluzione https://github.com/facebook/php-webdriver/wiki/HowTo-Wait)
	}
	...
	protected function dragFileToUpload($drop_area, $file) {
		...
		$return = $wd->executeScript($this->get_js_contents(self::JS_DRAG_SCRIPT), [$drop_area]);

		self::$climate->info('Waiting the js script execution...');
		$wd->manage()->timeouts()->implicitlyWait(3); // FIXME: non compatibile con Safari (soluzione https://github.com/facebook/php-webdriver/wiki/HowTo-Wait)
	...
	protected function clearBrowserConsole() {
		if (self::$browser == self::CHROME) {
			// Write avaiable browser logs (works only on Chrome)
			$this->getWd()->manage()->getLog('browser');
		} else {
		...
		if (self::$browser == self::CHROME) {
			...
			if ($console_error > 0) {
				self::$climate
					->yellow()
					->blink()
					->out('Errors on console: ' . $console_error . '. Url is: ' . $wd->getCurrentURL());
	...
	private function shouldSkip($message) {
		$array = ['Error reading remote data. Status is 0'];
	 ...
	 *
	 * Some notes:
	 * \n (Line Feed) = LF (Line Feed) → Used as a new line character in Unix/Mac OS X
	 * \r\n (End Of Line) = CR (Carriage Return) + LF (Line Feed) → Used as a new line character in Windows
	 *
	 */
